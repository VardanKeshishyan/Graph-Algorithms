...
void washGraph(Graph &G);


VertexList dfs(const Graph& graph, Vertex startVertex);
//static VertexList dfsNeighbors(const Graph& graph, Vertex vertex);
VertexList bfs(const Graph& graph, Vertex startVertex);
string get_arg(int argc, char *argv[], string def);
static void dfsCheck(const Graph& graph, Vertex v, vector<bool>& visited, VertexList& result);
